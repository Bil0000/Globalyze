...
function buildServerRuntimeContents(
  ...
  const supportedLocales = JSON.stringify(config.languages);
  ...
  if (!isTypeScript) {
    return [
      ...
      "  return normalizeLocale(match ? decodeURIComponent(match.slice(COOKIE_KEY.length + 1)) : null);",
      "}",
      "",
      "function readServerSideLocale() {",
      ...
      "      return cookieLocale;",
      "    }",
      "    const headerStore = typeof nextHeaders.headers === \"function\" ? nextHeaders.headers() : null;",
      "    const nextIntlLocale = normalizeLocale(headerStore?.get?.(\"x-next-intl-locale\"));",
      "    if (nextIntlLocale) {",
      ...
      "        const [localeCandidate] = entry.trim().split(\";\");",
      ...
      "  }",
      "",
      "  const serverLocale = readServerSideLocale();",
      "  if (serverLocale) {",
      "    return serverLocale;",
      "  }",
      "",
      ...
      "      return cookieLocale;",
      "    }",
      "    const documentLocale = normalizeLocale(typeof document !== \"undefined\" ? document.documentElement?.lang : null);",
      "    if (documentLocale) {",
      "      return documentLocale;",
      "    }",
      "    const browserLocale = normalizeLocale(window.navigator?.language ?? window.navigator?.languages?.[0]);",
      "    if (browserLocale) {",
  ...
  return [
    ...
    "",
    `const DEFAULT_LOCALE = ${defaultLocale};`,
    `const SUPPORTED_LOCALES = ${supportedLocales} as const;`,
    'const STORAGE_KEY = "globalyze.locale";',
    'const COOKIE_KEY = "globalyze.locale";',
    "",
    ...
    "",
    "  const language = trimmed.split(/[-_]/)[0];",
    "  return (SUPPORTED_LOCALES as readonly string[]).includes(language) ? language : null;",
    "}",
    "",
    ...
    "}",
    "",
    "function readServerSideLocale(): string | null {",
    '  if (typeof window !== "undefined") {',
    "    return null;",
    "  }",
    "",
  ...
  if (adapter.name === "next-intl") {
    if (!isTypeScript) {
      ...
      "      if (typeof window === \"undefined\") {",
        ...
        "      router.replace(`${nextPath}${nextQuery ? `?${nextQuery}` : \"\"}${nextHash}`);",
        "      router.refresh();",
        "    }",
        "  };",
        "}",
